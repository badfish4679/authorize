...
class curl {
    var
    $ch,$agent,$error,$info,$cookiefile;
    function curl()
    {
        $this->agent = get_agent(rand(0, 44));
        $this->ch = curl_init();
    ...
    }		function cookiefile($cookie_file_path)
    {
        $this->cookiefile = $cookie_file_path;
        curl_setopt($this->ch, CURLOPT_COOKIEJAR, $this->cookiefile);
        curl_setopt($this->ch, CURLOPT_COOKIEFILE, $this->cookiefile);
    }		function referer($ref)
    {
    ...
    {
        if ($this->cookiefile) unlink($this->cookiefile);
        curl_close($this->ch);
