...
function isVerified($Socks) {
	...
	$url="https://www.paypal.com/us/cgi-bin/webscr?cmd=_login-done";
...
function curl_get($url, $postfields=false,$Socks) {
	...
	if(empty($curl)) {
		...
		curl_setopt($curl, CURLOPT_USERAGENT, $this->get_agent(rand(0,44)));
		...
		curl_setopt($curl, CURLOPT_PROXYTYPE, CURLPROXY_SOCKS5);
			//curl_setopt($curl, CURLOPT_TIMEOUT_MS,$paypaltimeout);
			//curl_setopt($curl, CURLOPT_CONNECTTIMEOUT_MS,$paypaltimeout);
		curl_setopt($curl, CURLOPT_TIMEOUT,$paypaltimeout);
		curl_setopt($curl, CURLOPT_CONNECTTIMEOUT,$paypaltimeout);
...
#-------------------------------------------Check Sock--------------------------------------------
	...
	function Check($Socks,$paypaltimeout)
	{
			...
			curl_setopt($curl, CURLOPT_CONNECTTIMEOUT,$paypaltimeout);
			curl_setopt($curl, CURLOPT_USERAGENT, $this->get_agent(rand(0,44)));
			curl_setopt($curl, CURLOPT_URL, "https://www.paypal.com/cgi-bin/webscr?cmd=_xclick&business=paypal@dreamhost.com&i tem_name=Web+Hosting+Donation&item_number=donation _11557&amount=20.00&image_url=https://secure.newdream.net/dreamhostpp.gif&no_shipping=1&no_note=1&rm=2&retur n=http://www.dreamhost.com/donate.cgi&cancel_return=http://www.manutd.mu/&currency_code=USD&tax=0&submit=Donate+USD+%2420.0 0");
			$Exec = curl_exec($curl);
			//Print ($Exec);
		if ($Exec)
		{
			if(stristr($Exec,"credit card")){
				if(stristr($Exec,"Create PayPal Password")){
					$Paypal =  "Blacklist";
					}
				else{
					$Paypal =  "IP Clear Paypal";
					}
					}
			else{
					$Paypal =  "<font color='red'><b>Error, please contact Y!M: vsb_unface, Thanks.</b></font>";
					}
		}
		else $Paypal = "<font color='red'><b>Connect to Paypal timeout.</b></font>";
